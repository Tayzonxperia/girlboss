import { mongoose, prefix, sendresponse, parsecommand, approveprotedit, denyprotedit, getprotreq, listprotreqs, formatreqmessage, requestprotedit } from '../modulecontext.js'
            ...
            execute: async (envelope, message) => {
                ...
                if (sub === 'list') {
                    try {
                        ...
                        if (reqs.length === 0) {
                            await sendresponse('You have no pending protected property requests.', envelope, `${prefix}lockbox`, false);
                            return;
                        }
                        ...
                        await sendresponse(`Open auth requests (${reqs.length}):\n\n${lines.join('\n\n')}\n\nUse "${prefix}lockbox view <token>" for full details.`, envelope, `${prefix}lockbox`, false);
                    } catch (err) {
                        console.error(err);
                        await sendresponse('Failed to list requests. Please try again later.', envelope, `${prefix}lockbox`, true);
                    }
                } else if (sub === 'view') {
                    ...
                    try {
                        ...
                        if (!meta) {
                            await sendresponse('No pending request found for that token. It may have expired or already been handled.', envelope, `${prefix}lockbox`, true);
                            return;
                        }
                        if (meta.userid !== envelope.sourceUuid) {
                            await sendresponse('That request does not belong to you.', envelope, `${prefix}lockbox`, true);
                            return;
                        }
                        await sendresponse(formatreqmessage(token, meta, prefix), envelope, `${prefix}lockbox`, false);
                    } catch (err) {
                        console.error(err);
                        await sendresponse('Failed to retrieve request. Please try again later.', envelope, `${prefix}lockbox`, true);
                    }
                } else if (sub === 'approve') {
                    const token = match[2] ? match[2].toUpperCase() : null;
                    if (!token) {
                        await sendresponse(`Usage: ${prefix}lockbox approve <token>`, envelope, `${prefix}lockbox`, true);
                        return;
                    }
                    try {
                        ...
                        if (!result.ok) {
                            await sendresponse(`Could not approve request: ${result.reason}`, envelope, `${prefix}lockbox`, true);
                            return;
                        }
                        await sendresponse('Request approved.', envelope, `${prefix}lockbox`, false);
                    } catch (err) {
                        console.error(err);
                        await sendresponse('Failed to approve request. Please try again later.', envelope, `${prefix}lockbox`, true);
                    }
                } else if (sub === 'deny') {
                    const token = match[2] ? match[2].toUpperCase() : null;
                    if (!token) {
                    ...
                    try {
                        ...
                        if (!result.ok) {
                            await sendresponse(`Could not deny request: ${result.reason}`, envelope, `${prefix}lockbox`, true);
                            return;
                        }
                        await sendresponse('Request denied.', envelope, `${prefix}lockbox`, false);
                    } catch (err) {
                        console.error(err);
                        await sendresponse('Failed to deny request. Please try again later.', envelope, `${prefix}lockbox`, true);
                    }
                } else if (sub === 'test') {
                    try {
                        const user = await User.findOne({ userid: envelope.sourceUuid });
                        ...
                        await sendresponse(
                            `A test lockbox request has been created!\nUse "${prefix}lockbox list" to see it, or "${prefix}lockbox approve <token>" / "${prefix}lockbox deny <token>" to handle it.`, envelope, `${prefix}lockbox`, false
                        );
                    } catch (err) {
                        console.error(err);
                        await sendresponse('Failed to create test request. Please try again later.', envelope, `${prefix}lockbox`, true);
                    }
                } else {
                    await sendresponse(`Unknown subcommand "${sub}". Run "${prefix}lockbox" for help.`, envelope, `${prefix}lockbox`, true);
                }
            }
        }
    }
