        ...
        wallet: {
            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    await sendresponse(
                        'Failed to retrieve your wallet balance. Please try again later.',
                        envelope,
                        `${prefix}wallet`,
                        true
                    )
        ...
        daily: {
            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    if (ct - daily < cd) {
                        await sendresponse(
                            'You can only claim your daily reward once every 18 hours.',
                            envelope,
                            `${prefix}daily`,
                            true
                        )
                    ...
                    await user.save()
                    await sendresponse(
                        `You have claimed your daily reward of E${reward}! Your new balance is E${user.properties.eco.balance}.`,
                        envelope,
                        `${prefix}daily`,
                        false
                    )
                    ...
                    await sendresponse(
                        'Failed to claim your daily reward. Please try again later.',
                        envelope,
                        `${prefix}daily`,
                        true
                    )
        ...
        work: {
            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    const earnings = Math.floor(Math.random() * 20) + 5
                    ...
                    await user.save()
                    await sendresponse(
                        `You worked hard and earned E${earnings}! Your new balance is E${user.properties.eco.balance}.`,
                        envelope,
                        `${prefix}work`,
                        false
                    )
        ...
        give: {
            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    if (!match) {
                        await sendresponse(
                            'Invalid arguments. Use "-give [user] [amount]" to give money to another user.',
                            envelope,
                            `${prefix}give`,
                            true
                        )
                    ...
                    if (!user || !user.properties || !user.properties.eco || user.properties.eco.balance < amount) {
                        await sendresponse(
                            'You do not have enough balance to give this amount.',
                            envelope,
                            `${prefix}give`,
                            true
                        )
                    ...
                    await sendresponse(
                        `You have given E${amount} to ${tu.userid}. Your new balance is E${user.properties.eco.balance}.`,
                        envelope,
                        `${prefix}give`,
                        false
                    )
        ...
        leaderboard: {
            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    console.error(err)
                    await sendresponse(
                        'Failed to display leaderboard. Please try again later.',
                        envelope,
                        `${prefix}leaderboard`,
                        true
                    )
