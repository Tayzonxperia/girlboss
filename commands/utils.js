            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    if (action === 'generate') {
                        ...
                        if (!existingwebhook) {
                            await sendresponse(
                                "You don't have a webhook to delete.",
                                envelope,
                                `${prefix}webhook`,
                                true
                            )
                        ...
                        await existingwebhook.deleteOne()
                        await sendresponse(
                            'Your webhook has been deleted successfully.',
                            envelope,
                            `${prefix}webhook`,
                            false
                        )
                        ...
                        if (!existingwebhook) {
                            await sendresponse(
                                `You don't have a webhook yet. Use "${prefix}webhook generate" to create one.`,
                                envelope,
                                `${prefix}webhook`,
                                true
                            )
                        ...
                        await sendresponse(
                            `Invalid action. Available actions: generate, delete, status`,
                            envelope,
                            `${prefix}webhook`,
                            true
                        )
                    ...
                    await sendresponse(
                        'Failed to read/write data for this command. Please try again later.',
                        envelope,
                        `${prefix}webhook`,
                        true
                    )
        ...
        dig: {
            ...
            execute: async (envelope, message) => {
                try {
                    ...
                    if (!match) {
                        await sendresponse(
                            'Invalid arguments.\nUse "-dig [host] [type]" to get DNS info about a host.',
                            envelope,
                            `${prefix}dig`,
                            true
                        )
                    ...
                    if (!host) {
                        await sendresponse(
                            'You must provide a host to look up.\nUsage: -dig [host] [type]',
                            envelope,
                            `${prefix}dig`,
                            true
                        )
                    ...
                    switch (recordtype) {
                        ...
                        default:
                            await sendresponse(
                                `Unsupported record type: ${recordtype}\nSupported types: A, AAAA, MX, TXT, CNAME, NS, PTR, SOA, SRV, ANY`,
                                envelope,
                                `${prefix}dig`,
                                true
                            )
                    ...
                    if (!result || (Array.isArray(result) && result.length === 0) || (afr && afr === true)) {
                        await sendresponse(
                            `No ${recordtype.toUpperCase()} records found for ${host}`,
                            envelope,
                            `${prefix}dig`,
                            true
                        )
                    ...
                    if (err.code === 'ENOTFOUND') {
                        ...
                        await sendresponse(
                            'Failed to retrieve DNS information. Please try again later.',
                            envelope,
                            `${prefix}dig`,
                            true
                        )
                    ...
                    if (!id) {
                        ...
                        if (result) {
                            ...
                            if (match[2] && match[2].toLowerCase().trim() === 'true') {
                                ...
                                await sendresponse(
                                    `Please let the provider know that you have accepted the SSO request to finish logging in!`,
                                    envelope,
                                    `${prefix}sso`,
                                    false
                                )
