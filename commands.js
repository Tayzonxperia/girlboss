...
/** @typedef {import('./botenvelope.types.js').Envelope} Envelope */
    ...
    register: {
        ...
        execute: async (envelope, message) => {
            ...
            try {
                ...
                if (searchuser) {
                    await sendresponse(
                        `You are already registered as a ${botname} user $MENTIONUSER.`,
                        envelope,
                        `${prefix}register`,
                        true
                    )
                ...
                await user.save()
                await sendresponse(
                    `You are now registered as a ${botname} user $MENTIONUSER!`,
                    envelope,
                    `${prefix}register`
                )
                if (!contact) {
                    sendmessage(
                        `Hiya user!\nIt seems you have registered for a ${botname} account without sending me a DM first.\nThat's okay if so!\nPlease accept this message request so I can get to know you better.`,
                        envelope.sourceUuid,
                        phonenumber
                    )
                    return
                ...
                await sendresponse(
                    'Unable to connect to database, is MongoDB running?',
                    envelope,
                    `${prefix}register`,
                    true
                )
    ...
    unregister: {
        ...
        execute: async (envelope, message) => {
            try {
                ...
                await sendresponse(
                    `You are no longer registered as a ${botname} user $MENTIONUSER.`,
                    envelope,
                    `${prefix}unregister`
                )
            } catch (err) {
        ...
        execute: async (envelope, message) => {
            try {
                ...
                if (!match[1] || !match[1].trim()) {
                    await sendresponse(
                        `Invalid argument.\nUse "-subscribe true" or "-subscribe false" to subscribe or unsubscribe from ${botname} broadcasts.`,
                        envelope,
                        `${prefix}subscribe`,
                        true
                    )
                    ...
                    await user.save()
                    await sendresponse(
                        `You are now subscribed to ${botname} broadcasts $MENTIONUSER!`,
                        envelope,
                        `${prefix}subscribe true`,
                        false
                    )
                    ...
                    await user.save()
                    await sendresponse(
                        `You are now unsubscribed from ${botname} broadcasts $MENTIONUSER!`,
                        envelope,
                        `${prefix}subscribe false`,
                        false
                    )
                } else {
                    await sendresponse(
        ...
        execute: async (envelope, message) => {
            try {
                ...
                for (const prop in properties) {
                    ...
                    } else if (Object.prototype.hasOwnProperty.call(properties, prop)) {
                ...
                await sendresponse(
                    'Failed to retrieve your properties. Please try again later.',
                    envelope,
                    `${prefix}getprops`,
                    true
                )
    ...
    nick: {
        ...
        execute: async (envelope, message) => {
            try {
                ...
                await user.save()
                await sendresponse(
                    `Your nickname has been set to "${nickname}" $MENTIONUSER!`,
                    envelope,
                    `${prefix}nick`,
                    false
                )
                ...
                await sendresponse(
                    'Failed to set your nickname. Please try again later.',
                    envelope,
                    `${prefix}nick`,
                    true
                )
            ...
            try {
                ...
                if (envelope.dataMessage) {
                    ...
                    if (groupInfo && groupInfo.groupId) {
                        await sendresponse(
                            `Please check your DMs $MENTIONUSER for your AuthKey.`,
                            envelope,
                            `${prefix}authkey`,
                            true
                        )
                        sendmessage(am, envelope.sourceUuid, phonenumber)
                    } else {
                ...
                await sendresponse(
                    'Failed to create AuthKey. Please try again later.',
                    envelope,
                    `${prefix}authkey`,
                    true
                )
        ...
        execute: async (envelope, message) => {
            try {
                ...
                if (!match || !match[1] || match[1].trim().length === 0) {
                    await sendresponse(
                        'Please provide a valid feature request after the command.',
                        envelope,
                        `${prefix}featurereq`,
                        true
                    )
                ...
                await sendresponse(
                    `Your feature request has been submitted with ID ${reqid}.\nThank you!`,
                    envelope,
                    `${prefix}featurereq`,
                    false
                )
                const displayname =
                    user.properties && user.properties.nickname ? user.properties.nickname : user.username
                sendmessage(
                    `New feature request from ${displayname} (${envelope.sourceUuid}):\nRequest ID: ${reqid}\nFeature: ${feature}`,
                    managedaccount,
                    phonenumber
                )
            } catch (err) {
            ...
            try {
                ...
                if (/^\d+$/.test(pollidoridx)) {
                    ...
                    if (index >= 0 && index < polls.length) {
                        ...
                        await sendresponse(
                            `Poll index ${index} not found. Use "-poll" to see available polls.`,
                            envelope,
                            `${prefix}poll`,
                            true
                        )
                    ...
                    if (!poll) {
                        await sendresponse(
                            `Poll with ID ${pollidoridx} not found. Use "-poll" to see available polls.`,
                            envelope,
                            `${prefix}poll`,
                            true
                        )
                ...
                if (isNaN(vin) || vin < 0 || vin >= poll.options.length) {
                    await sendresponse(
                        'Invalid vote. Please provide a valid option number.',
                        envelope,
                        `${prefix}poll`,
                        true
                    )
                ...
                await poll.save()
                await sendresponse(
                    `Your vote for "${poll.options[vin]}" has been recorded.`,
                    envelope,
                    `${prefix}poll`,
                    false
                )
                ...
                await sendresponse(
                    'Failed to retrieve or vote on the poll. Please try again later.',
                    envelope,
                    `${prefix}poll`,
                    true
                )
    ...
    module: {
        ...
        execute: async (envelope, message) => {
            try {
                ...
                if (!match) {
                    await sendresponse(
                        'Invalid arguments. Use "-module [module] [enable/disable] [confirm: true/false]" to enable or disable a module.',
                        envelope,
                        `${prefix}module`,
                        true
                    )
                ...
                const ensureusertags = () => {
                    if (!user.properties) user.properties = {}
                    if (!user.properties.tags) user.properties.tags = []
                }
                const moduleexists = (section) => avamods.some((m) => m.section === section)
                if (!match[1]) {
                    if (avamods.length === 0) {
                ...
                if (!match[2]) {
                    ...
                    if (!moduleexists(module)) {
                        await sendresponse(
                            `Module "${module}" doesn't appear to exist.`,
                            envelope,
                            `${prefix}module`,
                            true
                        )
                    ...
                    await sendresponse(
                        `Module "${module}" is ${enabled ? 'enabled' : 'disabled'}.`,
                        envelope,
                        `${prefix}module`,
                        false
                    )
                ...
                if (action === 'enable' || action === 'on' || action === 'true') {
                    ...
                    if (mod && mod.execute && confirm && confirm === 'true') {
                        await sendresponse(
                            `Executing setup script for module ${module}...`,
                            envelope,
                            `${prefix}module`,
                            false
                        )
                ...
                console.error(err)
                await sendresponse(
                    'Failed to manage modules. Please try again later.',
                    envelope,
                    `${prefix}module`,
                    true
                )
    ...
    requestmydata: {
        ...
        execute: async (envelope, message) => {
            try {
                ...
                if (!user) {
                    await sendresponse(
                        `You are not registered as a ${botname} user $MENTIONUSER.\nUse "-register" to register!`,
                        envelope,
                        `${prefix}requestmydata`,
                        true
                    )
                    return
                }
                ...
                if (
                    userObj &&
                    userObj.properties &&
                    Object.prototype.hasOwnProperty.call(userObj.properties, 'authkey')
                ) {
                    userObj.properties.authkey =
                        'Ask r1sk.01 for the way to derive your raw key in the database from the one you receive.'
                }
                ...
                if (envelope.dataMessage) {
                    ...
                    if (groupInfo && groupInfo.groupId) {
                        await sendresponse(
                            `Please check your DMs $MENTIONUSER for your data.`,
                            envelope,
                            `${prefix}requestmydata`,
                            true
                        )
                ...
                await sendresponse(
                    "Failed to retrieve your data. Please contact r1sk.01 if this isn't a one-off, else, try again.",
                    envelope,
                    `${prefix}requestmydata`,
                    true
                )
...
const builtincommands = {
    ping: {
        ...
        execute: async (envelope, message) => {
            try {
                ...
                await sendresponse(
                    `It took ${timetaken}ms for ${botname} to execute this command.\nMethod: ${time} - ${timestamp} = ${timetaken}`,
                    envelope,
                    `${prefix}ping`,
                    false
                )
        ...
        execute: async (envelope, message) => {
            try {
                ...
                if (!section) {
                    ...
                    const visiblemodules = modules.filter((s) => {
                        if (s.admin && (!user || user.accesslevel !== 1)) return false
                        return !(s.user && !user)
                    })
                    if (!so || (so.admin && (!user || user.accesslevel !== 1))) {
                        helpmessage += `Unknown help module "${section}". Available modules: ${visiblemodules.map((s) => s.section).join(', ')}`
                ...
                if (envelope.dataMessage) {
                    ...
                    if (groupInfo && groupInfo.groupId) {
                        await sendresponse(
                            `Please check your DMs $MENTIONUSER for the help message.`,
                            envelope,
                            `${prefix}help`,
                            true
                        )
                        sendmessage(helpmessage, envelope.sourceUuid, phonenumber)
                    } else {
    ...
    info: {
        ...
        execute: async (envelope, message) => {
            try {
                const pkg = /** @type {{ name?: string, version?: string, license?: string, author?: string }} */ (
                    parseJsonc(fs.readFileSync('./package.json', 'utf8')) || {}
                )
                const appname = pkg.name || botname
    ...
    resolveid: {
        ...
        execute: async (envelope, message) => {
            ...
            try {
                ...
                if (!mention) {
                    await sendresponse(
                        'Invalid arguments.\nUse "-resolveid <@mention>" to resolve a Signal ID.',
                        envelope,
                        `${prefix}resolveid`,
                        true
                    )
                ...
                if (!mention.uuid) {
                    await sendresponse(
                        'Invalid mention. Please mention a user or provide a valid UUID.',
                        envelope,
                        `${prefix}resolveid`,
                        true
                    )
                ...
                await sendresponse(
                    `User ID for $MENTION_${mention.uuid} is ${mention.uuid}.`,
                    envelope,
                    `${prefix}resolveid`,
                    false
                )
...
async function invokecommand(command, envelope, self = false) {
    ...
    try {
        ...
        if (st && st.enabled === true) {
            const User = mongoose.model('User')
            const u = await User.findOne({ userid: ev.sourceUuid })
            const isadmin = u && u.accesslevel === 1
            if (!isadmin) {
                const cmdname = (command.startsWith(prefix) ? command.slice(prefix.length) : command)
                    .split(' ')[0]
    ...
    const propercommand = (command.startsWith(prefix) ? command.slice(prefix.length) : command)
        .split(' ')[0]
        .replace(/^./, (c) => c.toLowerCase())
    const User = mongoose.model('User')
    const user = await User.findOne({ userid: ev.sourceUuid })
    if (!self && user && ev.sourceName && ev.sourceName !== user.username) {
        user.username = ev.sourceName
        user.markModified('username')
        await user.save().catch((err) => console.error('Failed to save username:', err))
    }
    if (!self && propercommand === '') {
        if (ev.dataMessage && !ev.dataMessage.groupInfo) {
    ...
    if (located) {
        ...
        if (!mod.alwayson && mod.user && !mod.admin) {
            ...
            if (!tags.includes(mod.section)) {
                await sendresponse(
                    `Unknown command: ${command}\n(Hint: this command is part of the "${mod.section}" module, enable it with "${prefix}module ${mod.section} enable")`,
                    envelope,
                    command,
                    true
                )
