...
function isActionResponse(data: unknown): data is ActionResponse {
...
export function App() {
    ...
    const displaymessage = async (message: string, iserror: boolean) => {
    ...
    const fetchinfo = async () => {
        try {
            ...
            const data: AuthInfoResponse = await res.json()
            setIsLoading(false)
            if (data.error && res.status === 500) {
                ...
                setIsFailed({ status: true, message: 'Unexpected response from authinfo endpoint.' })
            }
        } catch (err) {
            setIsFailed({ status: true, message: toErrorMessage(err) })
            setIsLoading(false)
        }
    }
    ...
    async function doactivity(activity: string) {
        if (!authInfo.userid) {
            await displaymessage('You are not logged in!', true)
            return
        }
        try {
            ...
            if (!isActionResponse(data)) {
                await displaymessage('Unexpected response from server.', true)
                return
            }
            if (data.error) {
                await displaymessage(data.error, true)
            } else if (data.message) {
                await displaymessage(data.message, false)
                await fetchinfo()
            } else {
                await displaymessage('Unexpected response from server.', true)
            }
        } catch (err) {
            await displaymessage(toErrorMessage(err), true)
        }
    }
                ...
                <Card className="bg-card/40 backdrop-blur-sm border-muted">
                    <CardContent ref={cardref} className="pt-6">
                        <h1
                            ...
                            style={{ fontFamily: "'JetBrainsMono NF', 'JetBrainsMono', monospace" }}
                        >
                            GirlbossWeb
                        </h1>
                        <p>
        ...
        <div className="container mx-auto p-8 text-center relative z-10">
            ...
            <Card className="bg-card/40 backdrop-blur-sm border-muted">
                <CardContent ref={cardref} className="pt-6">
                    <h1
                        ref={titleref}
                        className="text-4xl font-bold my-4 leading-tight"
                        style={{ fontFamily: "'JetBrainsMono NF', 'JetBrainsMono', monospace" }}
                    >
                        Welcome, {authInfo.properties.nickname ? authInfo.properties.nickname : authInfo.username}!
                    </h1>
                    <p>View and manage your Girlboss eco account here :3</p>
                    <Separator className="my-3 bg-neutral-400" orientation="horizontal" />
                    <p style={{ marginBottom: '8px' }}>
                        Estrogen: E{authInfo.properties.eco ? authInfo.properties.eco.balance : '0'}
                    </p>
                    <Button
                        ref={error}
                        variant="secondary"
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        <Button variant="secondary" size="hz" className="w-full" onClick={() => logout()}>
                            Log Out
                        </Button>
                    </div>
                </CardContent>
