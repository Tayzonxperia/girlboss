...
function makescopeduser(userdoc, accesslist) {
    ...
    const proxy = {
        ...
        get userid() { return userdoc.userid; },
    };
    return proxy;
}
...
function formatreqmessage(token, meta, prefix) {
    const accesslines = meta.access.map(e => `  - ${e.path} (${e.level === 'rw' ? 'read + write' : 'read only'})`).join('\n');
    const sourceline = meta.source ? `Requested by: ${meta.source.module} module, command: ${prefix}${meta.source.command}${meta.source.args ? ' ' + meta.source.args : ''}` : 'Requested by: unknown';
    ...
    if (typeof onfail === 'function') {
        setTimeout(async () => {
            ...
            if (still) {
                ...
                try { onfail('timeout'); } catch (e) { console.error('onfail(timeout) error:', e); }
            }
        }, ttl * 1000);
    }
...
async function denyprotedit(token, userdoc) {
    ...
    if (cbs && typeof cbs.onfail === 'function') {
        try { cbs.onfail('denied'); } catch (e) { console.error('onfail(denied) error:', e); }
    }
    return { ok: true };
};
